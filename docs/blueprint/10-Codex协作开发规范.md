...
当前协调任务是蓝图与开发 TODO 的控制面，只负责：

* 维护和校验 `docs/blueprint`
* 将已确认设计拆成 `docs/implementation` 中可派遣的开发 TODO
* 将开发任务交给其他任务或协作者执行
* 汇总实现结果、验证证据和新决策，并回写 TODO 与蓝图

当前协调任务默认不修改业务代码、不直接执行具体功能开发。下面描述的代码实现、测试、Review 和提交步骤属于被派遣的开发任务；只有用户明确改变当前任务范围时，才在当前任务执行开发。
...
如果实现发现设计冲突，任务改为 `设计待确认`，由当前协调任务修订蓝图后再派遣。
...
更新 `docs/implementation` 中的任务状态；设计变化同步回写 `docs/blueprint`
