...
PF05/06、PF08/09、PF10/10A、PF10B、PF11统一先兼容Industrial Platform原生身份/租户/权限、公共服务与页面，再将同一核心按需独立部署并嵌入其他MES。各阶段必须先完成[蓝图32§2.1～2.2](32-Industrial%20Platform%20Service%20Host与内部模块边界.md)的最小装配、必需/可选依赖、宿主身份/业务上下文适配、数据/初始化与页面入口清单，然后派生产；当前未建实施方案的阶段继续待细化。

排期前置与运行依赖分开：PF09不因排在PF08后而强制运行低代码设计器；PF10A不强制装ServerMonitor；PF11不强制装Label。共享Host中的模块裁剪必须先核对共同初始化生命周期，不能凭功能开关声称已具备单独安装能力。平台纵向链先验，已批准外部形态另验；不改变PF编号、不新建业务分支，不将规划边界当作已交付安装包。
...
| PF-05/PF-06 | PF06本轮开发及用户功能验收完成，后续专项明确延期 | 2026-09-14用户确认功能与一端断开后对端断开通过；网络/质量待项目云Docker部署，异常待Electron/Capacitor，三端待终端完成。TASK-011后续专项不作为当前返工阻断；协作复盘已落统一约定，见active/PF-06 |
| PF-06A/PF-10B | 核心规格已细化，待派遣/待实际目标核验 | 实施09A/13B；没有原生或Label工程 |
...
- PF-07使用一个阶段管理会话，Scheduler与Platform Health分开建模，作为同一PF批准范围整包派遣，内部按依赖连续执行。
...
阶段与任务统一分开记录设计就绪度和派遣状态，不再另外使用含义不清的“派遣中”或把待派遣等同于已就绪：
...
阶段尚未开始的“待启动”、讨论中的“设计中/设计待确认”是设计活动备注，对应待细化；发现设计冲突回退相关范围的就绪度并记录阻塞，实际派遣历史不抹除。PoC和生产范围分别核验就绪，未通过生产前置不派生产。
...
**当前进度：** `TASK-ID-001～023` 已完成并完成集成。PF-00 本地门禁包括后端 Release 构建、Identity 五工程测试、SystemData Contract 测试以及前端 typecheck/lint/unit/build；真实 PostgreSQL/Redis 联合登录 E2E 仍作为外部验收项保留。
...
**证据：** `docs/evidence/PF-00.md`；具体交付与验证标识由该evidence维护。
...
**状态：** 已完成并完成集成（2026-09-05，独立验收 PASS）
...
- 原五模块保留，新增 StateMachine 与 UnitOfMeasure；实施 06 调整为 `TASK-RD-001～010` 十个顺序步骤，计量单位先于 Metadata，仍不独立派遣或提交；
...
**状态：** 2026-09-07 已补充字段/接口/线框图规格；任务待派遣、就绪度待前置核验，未实施。
...
**状态：** 2026-09-14 本轮开发及用户功能验收完成，停止行为亦由用户实测确认；无已知未修复生产缺陷。用户明确将网络/质量留至云Docker部署，异常留至Electron/Capacitor，三端留至终端完成后验证；TASK-011后续专项延期，不重测已通过路径，不将延期项派回当前开发。协作复盘及实际集成边界见active/PF-06。
**Service Host：** 现有Collaboration.Service内的逻辑模块，复用服务级初始化/迁移/账本与可靠设施。
**主控会话：** 当前PF06主控会话。
**实施文档：** `docs/implementation/09-Industrial Platform RemoteAssistance开发实施方案.md`，明细见PF06-数据接口与页面规格。
**目标：** 已有一对一聊天内增加“共享我的屏幕”“请求对方共享”和独立“语音通话”；同一时刻一方共享屏幕，语音可单独或同时使用，分别授权/结束，文字附件继续可用。
**路线：** Vue＋现有CollaborationHub信令＋浏览器原生WebRTC，必要时TURN；Screego仅作固定版本同条件画质/延迟对照。
**依赖：** 当前聊天成员/身份/权限/审计公开契约，真实两端浏览器与网络。代码已存在不等于运行验收通过；按实际消费能力核验。
**内部任务：** 保留TASK-PF06-001～011；V3.0保持，001＋009已有部分探索；当前002最终三表/约束→003邀请端点→004共用媒体控制→005身份/审计→006聊天UI→007网络→008屏幕基线→010独立语音→011组合最终验收。语音字段/slot忙线/15个Hub方法/共用协商已写明，不先实现旧屏幕协议再重构；内部连续执行，不逐卡确认。
**执行约定：** 读取[开发协作与页面统一约定](../agents/开发协作与页面统一约定.md)最新版本；实际落点见PF06规格§6.2/7.3/8.4及TASK002/005～008/010～011：真实链前移、既有三端聊天/动态菜单回归、UTC读回、Shared/PerService目标与唯一私有配置；开发自验后稳定交独立验收，验收不改源码，主控不机械重复全套。
**首版排除：** 第三人/目录选人、双方同时共享、独立管理页、完整票据/多人租约平台、多引擎、录音录像、远控、摄像头/群会议/系统声音共享和原生包。

**完成门禁：**

- 两种邀请方向的角色正确，每次捕获由实际共享者主动选择；接受/拒绝/取消/超时/换人完整。
- 语音与屏幕分别维护会话事实/邀请授权，共用一对终端的连接；每次校验身份/双方/能力/端点，旧会话不重开。静音不等于挂断，停止屏幕/结束语音不互相误停，结束全部释放连接。
- 同条件Screego对照：1080p中文/代码清晰、拖动滚动跟手、30分钟稳定；记录实际参数和差距，不以接通代替质量。
- 真实动态源和接收计数证明停止；正常≤5s，静默失联为最后有效确认请求起点30s＋最多5s容差，恢复后旧共享不续播。关闭页面/断WebSocket/凭据到期不能单独通过。
- 目标direct/TURN、PC双向共享、PDA/Mobile观看与聊天回归；实际菜单/顶栏可发现，移动列表→会话、焦点/滚底/右键/未读及PDA Enter行为不回归；外部MES/多实例/原生按后续明确范围另验。
- 追加迁移/权限种子并验证旧库升级、UTC落库读回和Shared/PerService目标，保留用户配置；开发配置唯一入口，真实媒体与Development附件扫描跳过分别记录，不互相替代验收。

本次已启动同一PF06工作包，开发与验收顺序交接；G06-1/3待真实媒体验收，环境缺项不再阻塞已定业务代码实现；G06-2工作线已登记、实际接入随开发核对；质量与真实停止未通过时对应能力保持关闭，不影响现有聊天。
...
**任务：** TASK-PF06A-001～008：能力矩阵→Runtime→PC/PDA容器→更新→协作→设备交接→真机验收。2026-09-14用户追加TASK-PF06A-009：平台接入SQL Server 2019/2022，覆盖当前服务Provider/配置/迁移/仓储/初始化与运维，保留PostgreSQL/SQLite；详见[SQL Server补充规格](../implementation/details/PF06A-SQLServer2019-2022接入补充规格.md)。009待接入细化及G06A-SQL核验，尚未派遣，不自动切换现用数据库。
**完成门禁：** 一台真实 PDA、一种广播、相机扫码、一种 BLE；PC/Bundle/APK 各一次升级、兼容/签名/回退与业务保护；原生聊天/共享/换人恢复。Mobile 原生平台范围在 001 明确，未知不当完成。009另验真实SQL Server 2019/150、2022/160及SQL-A01～10，终端008通过不代替平台数据库支持。
...
**状态：** 核心数据/接口/页面线框已细化，待派遣/待平台兼容、客户设备与外部适配核验，未实施。
...
**宿主：** Label.Service先接平台身份/权限/File/Audit/初始化与页面，按需设备执行；同一核心再验外部MES嵌入和无宿主最小装配。独立部署仍需必要身份/数据库/文件/审计能力。Agent为辅助部署单元，Label是新增第八个规划核心Host。
**任务：** TASK-PF10B-001～010：平台兼容/外部边界/样本设备核验→平台优先装配→数据契约/绑定/快照→模板→任务调度→Windows/PDA执行→平台页面全链再外部/无宿主验证→恢复试点→验收。
...
| PF-05 Collaboration | 已派遣，最小依赖先实现并做合同测试 | 原开发与独立验收任务继续协作 | 蓝图 04/05/32/33 | [实施 08](../implementation/08-Industrial%20Platform%20Collaboration开发实施方案.md)及[执行补充](../implementation/details/PF05-前置契约与合规执行补充.md) | 001～008整体执行，见[active工作包](../tasks/active/PF-05.md) | 基线已通过；PF05功能、真实环境与2C4G仍待验收 |
| PF-06 RemoteAssistance | V3.0设计与详细任务已收敛，实测待核验 | 当前PF06主控会话 | 用户需求、当前公开代码契约与官方资料 | `docs/implementation/09-Industrial Platform RemoteAssistance开发实施方案.md` | 两独立业务状态、共用媒体连接、分项授权与结束 | 已纠偏续行正式业务实现；真实媒体/质量最终验收待完成 |
| PF-06A 终端运行时与客户端打包（含SQL Server补充） | 待派遣/待前置核验 | 未创建执行任务 | 蓝图 34及用户SQL Server追加要求 | [实施 09A](../implementation/09A-Industrial%20Platform终端运行时与客户端打包开发实施方案.md) | 001～008终端、009数据库兼容，均未派遣 | 无原生/真机或SQL Server验收 |
