...
| PF-05/PF-06 | PF05已派遣，按明确依赖顺序实现；PF06仍待派遣/前置核验 | 实施08及执行补充；PF06按实施09收敛为聊天内双人共享，保留质量/真实停止核验 |
...
**状态：** 2026-09-12按统一约定复核PF06 TODO；V3.0设计保持，待派遣，G06-1/3真实媒体技术验证与G06-2实际集成待核验，未实施。
...
**内部任务：** 保留TASK-PF06-001～011；V3.0固定001＋009有限技术验证→002最终三表/约束→003邀请端点→004共用媒体控制→005身份/审计→006聊天UI→007网络→008屏幕基线→010独立语音→011组合最终验收。语音字段/slot忙线/15个Hub方法/共用协商已写明，不先实现旧屏幕协议再重构；内部连续执行，不逐卡确认。
...
本轮仅文档复审与任务细化；没有实际派遣、PoC、开发、媒体测试或部署。G06-1/3待真实技术验证，G06-2待实际工作线/环境核对；质量与真实停止未通过时对应能力保持关闭，不影响现有聊天。
...
| PF-06 RemoteAssistance | V3.0设计与详细任务已收敛，实测待核验 | 当前PF06主控会话 | 用户需求、当前公开代码契约与官方资料 | `docs/implementation/09-Industrial Platform RemoteAssistance开发实施方案.md` | 两独立业务状态、共用媒体连接、分项授权与结束 | 001～011待派遣；001＋009先行验证，未实施 |
