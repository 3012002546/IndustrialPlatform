...
## PF06 收口与后续主控边界（2026-09-15）

当前主控仅完成协作复盘、已通过代码基线核验与已证实冗余清理；原独立入口开发/验收停止。错误的未提交入口尝试已撤除。随后按用户授权在 `ae22f6a` 上完成 6 个生产文件的限定清理（未提交），Release 全方案构建 0 警告/0 错误，后端 Collaboration 157 项、前端 189 项回归通过，详见[清理证据第 6 节](../evidence/PF06-基线验证与冗余清理-20260915.md#6-用户授权的小范围清理1506-完成验证)。平台通用单点登录（中等规模改动）和独立部署接入由用户依次开启新主控，不归当前 PF06 续修。入口语义、已有代码及未解决问题见[后续主控交接](../implementation/details/平台单页入口与独立部署-后续主控交接.md)；下文历史里程碑不能作为恢复旧独立入口实施的授权。

## 集成基线

2026-09-15 收束补记：用户确认本轮清理后 PC、Mobile 实测均无问题；最后提交卫生检查通过，源码未再改动。当前 17 个待提交文件（6 源码、3 测试、8 文档），暂存区为空，由用户准备提交。实测范围不扩展为后续 single/standalone 或云部署已完成，详见上述清理证据第 7 节。
...
- 2026-09-14 / PF-06A新增要求：用户要求当前项目接入SQL Server 2019/2022，已录入TASK-PF06A-009及[SQL Server补充规格](../implementation/details/PF06A-SQLServer2019-2022接入补充规格.md)。覆盖当前平台Provider、配置/拓扑、各服务迁移/仓储、初始化/运维与真实业务回归；保留PostgreSQL/SQLite。当前待接入细化和G06A-SQL核验，未派遣/未实现/未连接SQL Server；不切现用云库，终端只经API访问，001～008编号与原延期安排保持。
...
- 2026-09-11 / 文件开发策略更新：用户将上传/导出真实杀毒扫描移到合入main前验收；DevelopmentBypass已收敛到统一src/backend/appsettings.Development.local.json一处（当前开启），UnifiedHost/SystemData.Api不重复配置，仅真实Development可生效。扫描明细记录未扫描的开发放行，其他文件授权检查保留。此前实现fresh Release 0警告/0错误、文件相关40/40通过；本次仅配置收敛与校验。ClamAV缺环境不阻塞日常开发，不代表真实扫描已通过；main前在同一配置关闭开关并以新文件补验。未重启用户调试，旧Failed导出不自动恢复。
...
- PF-06：2026-09-14，本轮开发及用户功能复测完成，无已知未修复功能缺陷；用户确认一端断开另一端也断开，本轮停止复验关闭。网络/TURN、质量/30分钟按用户安排后续云Docker部署验证；异常待Electron/Capacitor，PC/PDA/Mobile待终端完成验证。TASK-011后续专项明确延期，非当前返工阻断。协作复盘已回写统一约定与PF06注意事项；实际数据库拓扑/最终集成边界见active/PF-06。未提交推送部署。
