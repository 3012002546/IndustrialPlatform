...
版本V1.0；2026-09-07；对应实施13B。派遣状态待派遣；就绪度待平台兼容/客户/设备/外部适配核验。


## 1. 类型、命名与基线

平台原生兼容是首要输入，遵循蓝图32§2.1～2.2。领域表、API与页面核心在平台/外部MES嵌入/无宿主三种装配中相同；平台使用现有身份、权限、File/Audit和初始化公开端口，外部仅替换经核验的适配。前置核验必须列出真实平台接口、身份/租户映射、最小部署和缺失能力行为，不能先建立独立账号/文件/审计表再反向适配平台。
...
| TASK | 精确输入与顺序 | 验收断言 |
| --- | --- | --- |
| PF10B-001 | §3样例+G10B-1/2/3，先核验平台接口和三模式最小装配，再固定客户字段/保密/设备适配，回写最终映射后派生产 | 缺字段、平台契约、客户映射/设备回执不明均显式阻塞，不交给开发猜 |
| PF10B-002 | §1/2.14，先平台Host/初始化/身份文件审计公开端口，再同核心外部/无宿主适配 | 平台真实登录/权限/File/Audit和初始化通过，再验不启动全平台可认证并持久审计；无同名账号降级、无空审计；真实私有配置不出库 |
...
| PF10B-008 | §5全部线框+§4 API，先平台模板/数据/作业/历史/设备纵向链，再外部MES和无宿主同链 | 平台真实菜单/权限/File/Audit；外部宿主身份/退出/主题导航；客户保密负例、copies/Item区分、主从选择、错误保留输入 |
| PF10B-009 | §2.14恢复/§3版本，备份回滚/重装/签名/受控试点 | 暂停→对账→恢复；旧路径停调度后才回切，无双路打印 |
| PF10B-010 | 主文L1～L8+本规格反例全链 | 先平台真实兼容全链，再外部MES/无宿主同链；三模式和真实设备证据分开，Mock不替实际出纸 |
...
| G10B-3 | 总控：平台身份/权限/File/Audit/菜单/初始化真实接口及三模式适配、最小模块/进程/数据库/依赖清单 | 平台兼容映射先明确，再给外部宿主与无宿主的确切组件/类型/配置/迁移；必需/可选能力和缺失行为齐全，不创建第二套账号/文件/审计体系；此门未闭合不能派002生产 |
