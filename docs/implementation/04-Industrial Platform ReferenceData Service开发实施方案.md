...
> **当前执行前置：** 可运行基线、统一前端第一批和 Identity 真实登录闭环全部完成后，才开发 ReferenceData 业务功能。现有服务骨架和 `/health` 不等于本阶段完成。

> **纵向交付要求：** 字典、配置、元数据和编码规则 API 必须在本阶段同步交付对应 PC 管理页面、权限、契约测试和关键路径 E2E，不再把页面推迟到独立前端阶段。
