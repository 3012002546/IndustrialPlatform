...
只写依赖能力、公开契约、环境和验收前置；代码分支/提交哈希及交付快照放evidence或总控运行记录，不写成Todo执行条件。引用其他实施文档用文件/章节，不复制其修订号。
...
读取蓝图32§2.1～2.2，先设计平台原生兼容，再列同一核心支持的独立部署/外部MES嵌入形态；未纳入范围的无宿主模式明确暂缓。必须填写以下项，不以“模块独立”一句话代替：

| 输入 | 本阶段必须给出的明确结果 |
| --- | --- |
| 平台兼容 | 真实身份/租户/权限、File/Audit、初始化、路由菜单/共享UI/Runtime接口及缺口 |
| 最小运行依赖 | 模块/进程/数据库清单，必需或可选，禁用与缺失行为；与阶段排期前置分开 |
| 宿主适配 | 外部身份映射/退出/业务上下文/页面嵌入；公共能力复用或等价适配，精确契约归属 |
| 同核心约束 | 同业务模型/API/迁移/权限语义；装配差异、共享账本裁剪限制及升级恢复 |
| 验收顺序 | 平台真实纵向链先验，再验已批准外部/无宿主链；真实客户缺口独立记录 |

不复制公共平台服务，不因产品独立强拆每个PF为微服务。生产派遣前补齐本表对应的详细字段、接口、页面与用例；Todo只记录前置条件，不绑定分支或代码版本。
...
必须先读[PF05起派遣前详细设计与页面验收规则](STANDARD-派遣前详细设计与页面验收.md)。本模板所有数据/API/页面设计在派遣前完成；不能用“编码时确定”填空。
...
必须提供完整字典：表名/Schema/归属，字段逻辑名与物理名，.NET/PG/SQLite/JSON类型，可空/默认/长度/精度/枚举、PK/FK/UQ/索引及对应查询、删除保留、迁移/回填/恢复；通用生命周期集中引用。不得仅列“主要字段”。
...
至少提供一组成功和失败JSON样例、字段规则和权限反例；UI字段→DTO→权威数据可追溯。长序号/版本和decimal序列化要避免前端精度丢失。
...
页面编码前必须画线框图，标PageId/路由/终端/视口/数据区/上下文/动作/加载归属/窄屏变化；共用模板可复用图但逐路由列差异。按黄金页/yefeng信息架构选择全宽、主从或树表，非CRUD不硬套表格。线框图不替代真实页面验收。
...
禁止只写“实现对应页面”。明确每区row-key、selection、toolbar-profile、业务按钮和工具分区；A清空B清空，旧请求不覆盖新选择；submit guard和抽屉busy防重复写。路由/API bootstrap/权限/版本化菜单seed/旧库应用/当前用户菜单必须走通。
...
- [ ] API、事件、类型和路由前后一致。
- [ ] 每个需求都有对应任务和验收，完整字段字典/API样例/线框图在派遣前已写好。
- [ ] 待派遣状态与设计就绪度分开，阻塞列明责任、输入、退出条件；生产任务不包含未决设计。
- [ ] 每个任务具备统一九字段。
- [ ] 任务依赖与执行记录编号一致。
- [ ] 历史证据与本轮验证严格区分。
- [ ] `git diff --check` 通过。
