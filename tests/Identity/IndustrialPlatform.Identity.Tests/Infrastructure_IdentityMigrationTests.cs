...
public sealed class IdentityMigrationTests : IDisposable
{
    ...
    public async Task ApplyPending_RunTwice_IsIdempotent_SeedCountsStable()
    {
        ...
        // Identity 33 项 + Collaboration 19 项 + RemoteAssistance 3 项 + SystemData 50 项 + ReferenceData 38 项、SYSTEM_ADMIN 系统角色、无默认用户;
        // 两个不可变目录种子账本记录,重复执行不新增。
        Assert.Equal(149, first.PermissionCount);
        Assert.Equal(1, first.RoleCount);
        Assert.Equal(149, first.RolePermissionCount);
