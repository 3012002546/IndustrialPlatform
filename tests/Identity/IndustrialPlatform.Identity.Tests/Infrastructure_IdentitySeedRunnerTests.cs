...
public sealed class IdentitySeedRunnerTests : IDisposable
{
    ...
    public async Task FirstRun_CreatesThreeLayerSeeds_AndReturnsOneTimeCredential()
    {
        ...
        // Identity 33 项 + Collaboration 19 项 + SystemData 50 项 + ReferenceData 38 项，SYSTEM_ADMIN 自动获得完整目录。
        Assert.Equal(146, await CountAsync("SELECT COUNT(*) FROM identity_permission"));
        Assert.Equal(1, await CountAsync("SELECT COUNT(*) FROM identity_role WHERE n_id = 'SYSTEM_ADMIN'"));
        Assert.Equal(146, await CountAsync("SELECT COUNT(*) FROM identity_role_permission"));
    ...
    public async Task UpgradeFromVersionOne_AddsSystemDataPermissionsToSystemAdmin()
    {
        ...
        Assert.Equal(96, await CountAsync("SELECT COUNT(*) FROM identity_permission"));
