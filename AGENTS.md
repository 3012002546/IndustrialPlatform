...
- 主工作树默认只供 Codex 总控和最终集成；PF-02 按下述用户授权例外执行。
- 每个 PF 工作线/执行智能体默认长期使用一个独立 Git worktree 和分支；不是每张任务新建一次。
- 当前固定工作线：PF-00 当前范围已合入，旧工作树已于 2026-09-02 按用户要求移除，未经新指派不得重建；PF-02 自 2026-09-02 起按用户授权仅使用主工作区 `D:\Code\Industrial Platform\IndustrialPlatform` 的 `develop`，开发与独立验收顺序交接，不并行改文件。旧 PF-02 工作树已移除，不重建或重新启用旧工作线；具体边界见当前 PF 工作包。
...
- 派遣前由Codex完成表/字段/类型/约束/索引、API与事件字段样例、状态与事务、页面线框图及验收映射；不得把这些设计空白交给编码执行者。
- 规则入口为 `docs/implementation/STANDARD-派遣前详细设计与页面验收.md`；详细字典可放实施目录details，但主文任务必须引用精确章节。
- 待派遣工作包放 `docs/tasks/pending/`；分开登记“派遣状态”和“设计就绪度”。生产范围有未闭合设计/契约门禁不得实际派遣。
- 每页编码前已有线框图；黄金页/yefeng信息结构、选择/加载归属、重复提交、菜单发现性和本地验收按共同规则执行。
- 仅PoC能解决的未知项先写好可执行核验范围，核验结果由总控先回写生产规格；内部TASK仍不逐卡派遣/提交。
