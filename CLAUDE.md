...
- 私有配置文件为 `src/backend/appsettings.Development.local.json`,必须保持在 `.gitignore` 中,禁止提交其中的服务器地址、账号、密码、SSH 信息及其他凭据。
- 当该文件存在且 `RemoteDevelopment.Enabled=true` 时,本地后端调试使用文件中配置的云服务器开发基础设施。
- 当该文件不存在或 `RemoteDevelopment.Enabled=false` 时,本地数据库默认使用 SQLite,不得因为缺少云服务器配置而阻塞基础调试。
- 当前会话除非用户主动明确要求,不得运行单元测试、集成测试、端到端测试或全量测试;代码修改仅做用户明确授权的非测试检查。
...
| TASK-ID-007 | 服务端 RBAC、权限缓存与用户上下文(§14/§18):Application `Authorization/`(`IAuthorizationDataStore` 显式租户校验、`IPermissionCache` 版本化缓存端口、`IPermissionEvaluator`/`PermissionEvaluator` 撤销 fail-closed→缓存命中快速裁决→DB 权威装载回填缓存,缓存不可用降级、授权数据/撤销不可用抛 `SecurityStoreUnavailableException`、`AuthorizationDenialReason`、`AuthorizationOptions` TTL 15m、`IAuthorizationDenialSink` 审计、`AuthenticationService` 登出全部/改密失效缓存);Infrastructure `AuthorizationDataStore`(权限集与登录 `AuthUser.PermissionNIds` 同源同序)+`PermissionCacheStore`(Redis 键 `identity:user/{permission}:{tenant}:{user}:v{ver}`,失效按 `:v*` 前缀 SCAN 删除);Api `PermissionRequirement`+`PermissionAuthorizationHandler`(读 sub/tenant_id/sid/ver,拒绝原因写 HttpContext.Items)+`PermissionPolicies`(前缀 `permission:`,17 个策略)+`AddIdentityAuthorization`;OnForbidden 按原因映射 401「401」/403 账号不可用/403 ID_PERMISSION_DENIED/503 ID_AUTH_SECURITY_STORE_UNAVAILABLE;BuildingBlocks Security `ICurrentUser` 改为仅暴露 `UserNId`(收敛 Guid? 偏差) | 2026-08-11 全量 build 0 警告 0 错误、test 422 通过 / 2 失败(2 失败为协作方未提交 `DevelopmentInfrastructureConfigurationTests` 数据拓扑 WIP,非本任务回归、不在 387 基线);Identity 289(Domain 133、Infra 61、Api 33、App 61、Contract 1);新增 32:Application 20(PermissionEvaluator 17 + 缓存失效 3)、Infrastructure 5(AuthorizationDataStore SQLite)、Api 7(全栈探针 401/403/503 信封);`--vulnerable` 25/25 项目干净;PostgreSQL/Redis 真实验证「待验收」 ✅ |

### 待实施 ⏳

| 任务 | 内容 |
| --- | --- |
| TASK-ID-008~016 | 管理 API、Outbox 集成事件、前端接入与管理页、SSO 与联合验收 |

### 会话进度快照(2026-08-11)

- **当前状态:** TASK-ID-001~007 已完成(TASK-ID-001~006 已提交 develop,最新提交记录为 TASK-ID-006 `cd46e3c`;TASK-ID-007 待提交,建议 `feat(identity): add tenant aware permission authorization`);全量 build 0 警告 0 错误、test 422 通过 / 2 失败(2 失败为协作方未提交 `DevelopmentInfrastructureConfigurationTests` 数据拓扑 WIP:`Apply` 的 Seq/RabbitMQ 新增 `Enabled` 门控与测试夹具无 `Enabled` 字段不一致,非本任务回归、不在 387 基线内);`--vulnerable` 25/25 项目干净;PostgreSQL/Redis 全链路(登录/刷新 E2E、撤销 fail-closed)「待验收」。
- **暂停点:** TASK-ID-007(服务端 RBAC、权限缓存与用户上下文)已完成,下一步 **TASK-ID-008(用户角色管理 API 与审计)**;工作区含协作方未提交的数据拓扑改造文件(`IndustrialPlatform.Web/Configuration/`、`appsettings.Development.local.example.json`、`deploy/cloud-dev/` 等),提交 TASK-ID-007 前需选择性暂存,避免裹入协作方 WIP。
