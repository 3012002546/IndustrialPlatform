...
public static class EmbeddedAccountQuery
{
    ...
    public static void EnsureMatches(EmbeddedIdentity identity, string? requestedAccountNId)
    {
        if (requestedAccountNId is not null && !string.Equals(identity.AccountNId, requestedAccountNId, StringComparison.Ordinal))
            throw new EmbeddedHandshakeException(StatusCodes.Status403Forbidden, "EMBEDDED_ACCOUNT_MISMATCH", "account 与当前 MES 登录用户不匹配。");
    }
}
...
public sealed class ConfigurationEmbeddedSourcePrincipalResolver(IConfiguration configuration, EmbeddedHostHandshakeOptions options) : IEmbeddedSourcePrincipalResolver
{
    public Task<EmbeddedSourcePrincipal?> ResolveAsync(HttpContext context, string? requestedAccountNId, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(sourceCookie))
        {
            ...
            if (configured is not null && MatchesConfiguredDemoAccount(configured, requestedAccountNId))
    ...
    private bool MatchesConfiguredDemoAccount(EmbeddedSourcePrincipal source, string? requestedAccountNId)
    {
        if (requestedAccountNId is null) return true;
        var account = configuration.GetSection($"EmbeddedCollaboration:Demo:Accounts:{requestedAccountNId}");
        var externalSubject = account["ExternalSubject"];
        return string.IsNullOrWhiteSpace(externalSubject)
            || string.Equals(externalSubject, source.ExternalSubject, StringComparison.Ordinal);
    }
...
/// <summary>根据 HttpOnly Cookie 恢复有效身份；会话或上游登录失效时拒绝认证。</summary>
