...
namespace IndustrialPlatform.SystemData.Domain.DatabaseOrchestration.Identities;

/// <summary>
/// SystemData 数据库编排业务标识值对象:不透明字符串,大小写不敏感,带格式校验与规范化。
/// 格式:1-128 个字符,以字母或数字开头,可含字母、数字、下划线、点与连字符。
/// 相等性基于规范化值,<c>plan-01</c> 与 <c>PLAN-01</c> 视为相同。
/// </summary>
public sealed partial class NId : ValueObject
{
    ...
    public static NId Create(string? value)
    {
        ...
        {
            throw new ValidationException("业务标识不能为空。");
        }

        var trimmed = value.Trim();
        if (trimmed.Length > MaxLength)
        {
            throw new ValidationException($"业务标识长度不能超过 {MaxLength} 个字符。");
        }

        if (!FormatPattern().IsMatch(trimmed))
        {
            throw new ValidationException("业务标识只能包含字母、数字、下划线、点与连字符,且必须以字母或数字开头。");
        }
