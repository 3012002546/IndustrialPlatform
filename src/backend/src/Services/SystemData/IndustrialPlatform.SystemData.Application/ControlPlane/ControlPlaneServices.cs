...
public sealed class ResourceNavigationService : IResourceNavigationService
{
    ...
    public async Task<NavigationRuntimeResponse> RuntimeAsync(string tenantNId, UiTerminal terminal, CancellationToken cancellationToken)
    {
        ...
        {
            var snapshot = state.ActiveSnapshotRevision is { } active ? state.Snapshots.FirstOrDefault(x => x.Revision == active) : null;
            ...
            if (snapshot is null) throw new ValidationException("Navigation is not published.");
            ...
            return new NavigationRuntimeResponse { Revision = snapshot.Revision, Nodes = BuildRuntime(nodes, null), Degraded = false };
        }, cancellationToken);
...
public sealed class ServiceCatalogControlService : IServiceCatalogControlService
{
    ...
    public async Task<ServiceCatalogRuntimeResponse> RuntimeAsync(string tenantNId, CancellationToken cancellationToken) { var result = await _runtime.LoadAsync(tenantNId, "service-catalog", state => new ServiceCatalogRuntimeResponse { Revision = state.Revision, Items = state.Catalog.Where(x => x.Status == ServiceCatalogStatus.Active).Select(ToResponse).ToArray() }, cancellationToken); return result.Value with { Degraded = result.Degraded, Items = result.Value.Items.Select(x => x with { Degraded = result.Degraded }).ToArray() }; }
...
public sealed class ThemePolicyControlService : IThemePolicyControlService
{
    ...
    public async Task<ThemePolicyResponse> RuntimeAsync(string tenantNId, CancellationToken cancellationToken) { var result = await _runtime.LoadAsync(tenantNId, "theme-policy", state => { var theme = state.Theme ?? throw new ValidationException("Theme policy was not configured."); return ToResponse(theme); }, cancellationToken); return result.Value with { Degraded = result.Degraded }; }
    ...
    private static ThemePolicyResponse ToResponse(ThemePolicy p) => new() { PolicyRevision = p.PolicyRevision, AllowedPalettes = p.AllowedPalettes.Select(x => x.ToString()).ToArray(), AllowedModes = p.AllowedModes.Select(x => x.ToString()).ToArray(), AllowedPcDensities = p.AllowedPcDensities.Select(x => x.ToString()).ToArray(), DefaultPalette = p.DefaultPalette.ToString(), DefaultMode = p.DefaultMode.ToString(), DefaultPcDensity = p.DefaultPcDensity.ToString() };
