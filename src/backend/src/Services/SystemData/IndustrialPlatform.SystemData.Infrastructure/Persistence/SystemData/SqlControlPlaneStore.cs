...
public sealed class SqlControlPlaneStore : IControlPlaneStore, IControlPlaneTenantSource
{
    ...
    private static NavigationNode ToNavigation(SystemDataNavigationTable x) { var terminals = Deserialize<UiTerminal[]>(x.VisibleTerminalsJson); var actionResourceNIds = Deserialize<string[]>(string.IsNullOrWhiteSpace(x.ActionResourceNIdsJson) ? "[]" : x.ActionResourceNIdsJson); var n = x.Kind.Equals(nameof(NavigationNodeKind.Group), StringComparison.OrdinalIgnoreCase) ? NavigationNode.CreateGroup(x.TenantNId, x.NodeNId, x.Label, x.ParentNodeNId, x.NavigationSetNId, x.IconKey) : NavigationNode.CreateLink(x.TenantNId, x.NodeNId, x.Label, x.ParentNodeNId, x.NavigationSetNId, x.ResourceNId!, x.FeatureNId, terminals, x.IconKey, actionResourceNIds); n.SetDisplayOrder(x.DisplayOrder); if (x.Status.Equals(nameof(NavigationNodeStatus.Inactive), StringComparison.OrdinalIgnoreCase)) n.Retire(); return n; }
    ...
    private static SystemDataNavigationTable ToNavigationTable(NavigationNode x, DateTimeOffset now) { var row = Base<SystemDataNavigationTable>(x.TenantNId, now); row.NodeNId = x.NId; row.NavigationSetNId = x.NavigationSetNId; row.ParentNodeNId = x.ParentNodeNId; row.Kind = x.Kind.ToString(); row.Label = x.Label; row.IconKey = x.IconKey; row.ResourceNId = x.ResourceNId; row.FeatureNId = x.FeatureNId; row.DisplayOrder = x.DisplayOrder; row.VisibleTerminalsJson = Serialize(x.VisibleTerminals); row.ActionResourceNIdsJson = Serialize(x.ActionResourceNIds); row.Status = x.Status.ToString(); return row; }
