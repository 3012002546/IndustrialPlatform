...
internal static class StreamingXlsxExport
{
    ...
    internal static int ParseQuantity(string quantity) =>
        string.Equals(quantity, "all", StringComparison.OrdinalIgnoreCase)
            ? int.MaxValue
    ...
    internal static FileStreamResult Start(
        string fileName,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new FileStreamResult(pipe.Reader.AsStream(leaveOpen: false), ContentType)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await producer(output, cancellationToken);
        }
        catch (Exception ex)
        {
            failure = ex;
