...
3. Codex 指派的 PF-00 工作包；
4. 工作包指向的 PF-00 实施方案，并按内部任务顺序读取当前所需章节。

只按 PF-00 工作包逐步读取当前内部任务需要的实施方案章节，不读取其他 PF 实施方案、历史 evidence 或旧任务卡。当前项目状态由 Codex 维护在 `docs/status/CURRENT.md`。

## 工作边界

- 长期使用 Codex 分配的 PF-00 worktree 和分支；连续任务复用该工作线，不为每张任务新建分支。
- 不得在主工作树工作，也不自行同步 `develop`；新任务开始前由 Codex 完成同步。
- 只在 PF-00 边界内工作；内部 `TASK-ID-*` 按依赖连续执行，不逐项等待 Codex 派遣。
...
- 对整个 PF-00 工作包的编码、测试、修复和内部验证负责；内部任务完成后直接继续下一项。
- 整个 PF-00 当前范围完成后再交付；Codex 只负责一次最终 Release 编译门禁。
- 读取退出码、失败数和警告数，不使用历史结果声称通过。
- 默认不暂存、不提交，保留完整未提交工作树交给 Codex 编译和提交。
- 整个 PF-00 使用 `docs/evidence/PF-00.md`，内部任务只追加简短进度。
