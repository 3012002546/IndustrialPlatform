...
平台基础能力已覆盖身份与权限、统一前端、系统配置、参考数据，以及文件、通知和审计的核心实现。协作与媒体功能已进入集成和部署验证阶段；独立协作宿主已有单实例接入、打包与 IIS 部署说明，实际环境的数据库写权限和会话链路仍需按部署文档验收。各阶段的完成范围与未验证事项以[当前状态](docs/status/CURRENT.md)、[阶段证据](docs/evidence/PF-06.md)和[独立部署说明](src/backend/src/Hosts/IndustrialPlatform.Collaboration.EmbeddedHost/README.md)为准；这里不把规划能力等同于已交付能力。
